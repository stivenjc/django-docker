...
class TaskTestcas(TestSetup):
    def test_get_all_task(self):
        response = self.client.get(self.url_task, format='json', headers=self.Token)
        self.assertGreaterEqual(len(response.data), 1)
        self.assertEqual(response.status_code, status.HTTP_200_OK)

    def test_get_only_task(self):
        response = self.client.get(f'{self.url_task}{self.task.id}/', format='json', headers=self.Token)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_can_not_remove_task_any(self):
        ...
        """
        response = self.client.delete(f'{self.url_task}{self.task.id}/', format='json', headers=self.Token)
    ...
    def test_remove_task_only_created_task(self):
        ...
        """
        response = self.client.delete(f'{self.url_task}{self.task_1.id}/', format='json', headers=self.Token)
    ...
    def test_create_only_five_task_for_project(self):
        ...
        response = self.client.post(f'{self.url_task}', task, format='json', headers=self.Token)
